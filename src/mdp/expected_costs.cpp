...
num_t MDP::expected_costs(size_t state1, size_t state2, size_t prodRate1, size_t prodRate2)
{
    ...
    for (size_t sdx1 = 0; sdx1 != (d_nStates-state1-1); ++sdx1) 
    {
        ...
        {
            costs += prop1 * d_prop1[prodRate2][sdx2] * d_valueB[hash(sdx1, sdx2)];
        }
        costs += prop1 * d_prop2[prodRate2][state2] * d_valueB[hash(sdx1, d_nStates)];
    }
    ...
    for (size_t sdx2 = 0; sdx2 != (d_nStates-state2-1); ++sdx2)
    {
        costs += prop1 * d_prop1[prodRate2][sdx2] * d_valueB[hash(d_nStates, sdx2)];
    }
    costs += prop1 * d_prop2[prodRate2][state2] * d_valueB[hash(d_nStates, d_nStates)];
